...
const RecordSection: React.FC = () => {
  ...
  return (
    <section className="mt-2 pb-20">
      {/* 현재 연도와 달 표시 */}
      <h2 className="text-center text-xl font-galmuri-11-bold text-zinc-600">
        {/* YYYY년 MM월 */}
        <span onClick={() => changeMonth("prev")}>&lt;</span>{" "}
        {`${yearMonth.slice(0, 4)}년 ${yearMonth.slice(4, 6)}월`}{" "}
      ...
      {petDatas?.map((pet: petRecordInterface) => (
        <PetRecord
          ...
          petImageUrl={
            petSpriteMetaData.find((sprite) => sprite.petCode === pet.petCode)
              ?.imageUrl || ""
          }
          ...
          typeCode={pet.typeCode} // 여기에 typeCode 추가
        />
      ))}
      ...
      <PetDetailModal
        ...
        petImageUrl={
          petSpriteMetaData.find(
            (sprite) => sprite.petCode === selectedPet.petCode
          )?.imageUrl || ""
        }
